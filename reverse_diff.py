...
#             self.declare_stmts.append(loma_ir.Declare('_stack_ptr_float', loma_ir.Int()))

#             for stmts in range(len(node.body)):
#                 if isinstance(node.body[stmts], loma_ir.Declare):
#                     self.declare_stmts.append(node.body[stmts])
#                     self.mutate_stmt(node.body[stmts])
#                     if node.body[stmts].val is not None:
#                         self.declare_stmts.append(loma_ir.Assign(loma_ir.ArrayAccess(loma_ir.Var('_t_float'),loma_ir.Var('_stack_ptr_float')),loma_ir.Var(node.body[stmts].target.id)))
...
#                 elif isinstance(node.body[stmts], loma_ir.Assign):
#                     if not check_lhs_is_output_args(node.body[stmts].target, self.output_args):
#                         self.declare_stmts.append(loma_ir.Assign(loma_ir.ArrayAccess(loma_ir.Var('_t_float'),loma_ir.Var('_stack_ptr_float')),loma_ir.Var(node.body[stmts].target.id)))
#                         self.declare_stmts.append(loma_ir.Assign(loma_ir.Var('_stack_ptr_float'),loma_ir.BinaryOp(loma_ir.Add(),loma_ir.Var('_stack_ptr_float'),loma_ir.ConstInt(1))))
#                         self.declare_stmts.append(node.body[stmts])
#                 # if not isinstance(node.body[stmts],loma_ir.Return):
#                 #     if not check_lhs_is_output_args(node.body[stmts].target, self.output_args):
#                 #         self.declare_stmts.append(node.body[stmts])
#                     self.mutate_stmt(node.body[stmts])
...
#                 # if isinstance(node.body[stmts], loma_ir.Assign):
#                 #     if not check_lhs_is_output_args(node.body[stmts].target, self.output_args):
#                 #         self.declare_stmts.append(loma_ir.Assign(loma_ir.Var('_d' + node.body[stmts].target.id), loma_ir.ConstFloat(0.0)))
...
#                 if arg.i == loma_ir.In():
#                     new_args.append(arg)
#                     new_args_id = '_d' + arg.id
#                     new_args.append(loma_ir.Arg(new_args_id,arg.t, loma_ir.Out()))
...
#             print(fm.total)
#             for i in range(fm.total):
#                 new_body_fwd.append(loma_ir.Declare(f'_adj_{i}', loma_ir.Float()))
            
#             #reverse pass: traverse body in reverse
...
#                 stmts1 = self.mutate_expr(node.val)
#                 stmts2 = loma_ir.Assign(loma_ir.Var('_d' + node.target), loma_ir.ConstFloat(0.0))
#                 self.adjoint = None
#                 self.flag_switch = 1
#                 self.counter = 0
#                 self.adjoint = loma_ir.Var('_d' + node.target)
#                 stmts3 = self.mutate_expr(node.val)
#                 self.adjoint = None
#                 self.flag_switch = 0
#                 return [stmts1]+[stmts2]+[stmts3]
#             return []
...
#                 return stmts1 + stmts2
#             else:                
#                 stmts1 = loma_ir.Assign(loma_ir.Var('_stack_ptr_float'),loma_ir.BinaryOp(loma_ir.Sub(),loma_ir.Var('_stack_ptr_float'),loma_ir.ConstInt(1)))
#                 stmts2 = loma_ir.Assign(loma_ir.Var(node.target.id), loma_ir.ArrayAccess(loma_ir.Var('_t_float'),loma_ir.Var('_stack_ptr_float')))
#                 self.adjoint = loma_ir.Var('_d' + node.target.id)
#                 stmts3 = self.mutate_expr(node.val)
#                 stmts4 = loma_ir.Assign(loma_ir.Var('_d' + node.target.id), loma_ir.ConstFloat(0.0))
#                 self.flag_switch = 1
#                 self.counter = 0
#                 stmts5 = self.mutate_expr(node.val)
#                 self.adjoint = None
#                 self.flag_switch = 0
#                 return [stmts1]+[stmts2]+[stmts3]+[stmts4] + [stmts5]
...
#                 stmts = [loma_ir.Assign(loma_ir.Var(f'_adj_{self.counter}'), self.adjoint)]
#             else:
#                 dx = loma_ir.Var('_d' + node.id)
#                 count = self.list1.pop(0)
#                 rhs = loma_ir.BinaryOp(loma_ir.Add(),dx, loma_ir.Var(f'_adj_{count}'))
#                 lhs = dx
#                 stmts = [loma_ir.Assign(lhs,rhs)]
#             self.counter +=1
#             return stmts
...
#                 old_adjoint = self.adjoint
#                 self.adjoint = loma_ir.BinaryOp(loma_ir.Add(), self.adjoint, loma_ir.ConstFloat(0.0))
#                 stmts_left = self.mutate_expr(node.args[0])
#                 self.adjoint = old_adjoint
...
def reverse_diff(diff_func_id : str,
    ...
    class RevDiffMutator(irmutator.IRMutator):
        ...
        def mutate_div(self, node):
            ...
            old_adj = self.adjoint
            t1 = loma_ir.BinaryOp(loma_ir.Mul(), node.left, self.adjoint)
            t2 = loma_ir.BinaryOp(loma_ir.Mul(), node.right, node.right)
            t3 = loma_ir.BinaryOp(loma_ir.Sub(), loma_ir.ConstFloat(0.0), t1)
            self.adjoint = loma_ir.BinaryOp(loma_ir.Div(), t3, t2)
            stmts_right = self.mutate_expr(node.right)
            self.adjoint = old_adj
        ...
        def mutate_call(self, node):
            ...
            if node.id == 'sin':
                ...
                old_adj = self.adjoint
                t1 = loma_ir.BinaryOp(loma_ir.Mul(), loma_ir.ConstFloat(-1.0), self.adjoint)
                self.adjoint = loma_ir.BinaryOp(loma_ir.Mul(), t1, loma_ir.Call('sin', node.args))
                stmts = self.mutate_expr(node.args[0])
                self.adjoint = old_adj
                return stmts
                ...
                old_adj = self.adjoint
                t1 = loma_ir.BinaryOp(loma_ir.Div(), loma_ir.ConstFloat(0.5), loma_ir.Call('sqrt', node.args))
                self.adjoint = loma_ir.BinaryOp(loma_ir.Mul(), self.adjoint, t1)
                stmts = self.mutate_expr(node.args[0])
                self.adjoint = old_adj
                return stmts
                ...
                old_adj = self.adjoint
                t1 = loma_ir.BinaryOp(loma_ir.Div(), loma_ir.ConstFloat(1.0), node.args[0])
                self.adjoint = loma_ir.BinaryOp(loma_ir.Mul(), self.adjoint, t1)
                stmts = self.mutate_expr(node.args[0])
                self.adjoint = old_adj
                return stmts
                ...
                old_adj = self.adjoint
                self.adjoint = loma_ir.BinaryOp(loma_ir.Mul(), self.adjoint, loma_ir.Call('exp', node.args))
                stmts = self.mutate_expr(node.args[0])
                self.adjoint = old_adj
                return stmts
                ...
                old_adj = self.adjoint
                t1 = loma_ir.BinaryOp(loma_ir.Mul(), loma_ir.Call('pow', node.args), loma_ir.Call('log', [node.args[0]]))
                self.adjoint = loma_ir.BinaryOp(loma_ir.Mul(), self.adjoint, t1)
                stmts1 = self.mutate_expr(node.args[1])
                self.adjoint = old_adj

                old_adj = self.adjoint
                t2 = loma_ir.BinaryOp(loma_ir.Mul(), loma_ir.Call('pow',[node.args[0], loma_ir.BinaryOp(loma_ir.Sub(), node.args[1], loma_ir.ConstFloat(1.0))]), node.args[1])
                self.adjoint = loma_ir.BinaryOp(loma_ir.Mul(), self.adjoint, t2)
                stmts2 = self.mutate_expr(node.args[0])
                self.adjoint = old_adj

                return stmts2 + stmts1
